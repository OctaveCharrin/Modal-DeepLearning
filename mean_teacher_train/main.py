...
def create_data_loaders(train_transformation,
    ...
    # datamodule = DataModule(traindir, train_transform=train_transformation, val_transform=eval_transformation, batch_size=256, num_workers=0)
...
# def print_images(batch_images):
#     # Convert tensor to numpy array
#     np_images = batch_images.numpy()

#     # Transpose the dimensions of the batch (assuming NCHW format)
#     transposed_images = np.transpose(np_images, (0, 2, 3, 1))

#     # Create a grid of subplots to display the images
#     num_images = len(transposed_images)
...
def validate(eval_loader, model, log, global_step, epoch):
    ...
    for i, (input, target) in enumerate(eval_loader):
        ...
        # input_var = input.to('cuda').detach().requires_grad_(False)
        with torch.no_grad():
