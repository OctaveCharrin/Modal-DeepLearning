...
class RunContext:
    ...
    def __init__(self, runner_file, run_idx):
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")  # Update the timestamp format
        self.result_dir = os.path.join('results', runner_name, timestamp, str(run_idx))
        self.transient_dir = os.path.join(self.result_dir, "transient")
